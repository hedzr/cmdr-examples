...
func tuiDemo(cmd *cmdr.Command, args []string) (err error) {
	...
	resizing := func(w, h int) {
		p.SetRect(0, 0, w-25, 5)
		bc.SetRect(w-25, 0, w, 5+7+3)

		l.SetRect(0, 5, 25, 12)
		slg.SetRect(25, 5, w-25, 12)
		g.SetRect(0, 12, w-25, 15)

		lc.SetRect(0, 15, w/2, 25)
		lc2.SetRect(w/2, 15, w, 25)
	}
	resizing(tb.Size())
...
const (
	appName   = "tui-demo"
	copyright = "tui-demo is an effective devops tool"
	desc      = "tui-demo is an effective devops tool. It make an demo application for `cmdr`."
	longDesc  = "tui-demo is an effective devops tool. It make an demo application for `cmdr`."
	examples  = `
