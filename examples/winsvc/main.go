...
// .\winsvc_windows_amd64.exe -service stop
// .\winsvc_windows_amd64.exe -service status
...
func simpleMain() {
	svcConfig := &service.Config{
		Name:        "hz-winsvc",              // 服务显示名称
		DisplayName: "HZ-winsvx",              // 服务名称
		Description: "hz-winsvc - nt service", // 服务描述
	}
...
func (p *programSimple) Start(s service.Service) error {
	go p.run()
	return nil
}

func (p *programSimple) run() {
	// 代码写在这儿
}

func (p *programSimple) Stop(s service.Service) error {
	return nil
}
